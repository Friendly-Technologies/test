...
public class LoopbackDiagnosticStrategy implements DiagnosticRootStrategy {
    @Override
    public String getDiagnosticRoot(ProtocolType protocol, Long deviceId, ParameterService parameterService) {
        if (protocol == USP) {
            return null;
        }
        List<String> params = parameterService.getParamNamesLike(deviceId, "%.WANDevice.%.WANATMF5LoopbackDiagnostics.");
    ...
    public String getDiagnosticRootByProductClass(ProtocolType protocol, Long groupId, TemplateService templateService) {
        ...
        List<String> params = templateService.getParamNamesLike(groupId, "%.WANDevice.%.WANATMF5LoopbackDiagnostics.");
        return params.isEmpty() ? null : params.get(0);
