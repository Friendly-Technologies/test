...
public class ExcelDeviceEventReportGeneratorStrategy implements ExcelReportGeneratorStrategy {
    ...
    public void generateReport(Session session, Map<String, Object> params, String fileName) {
        ...
        final Instant to = params.get("to") == null
                ? null
                : DateTimeUtils.clientToServer(Instant.parse((String) params.get("to")), clientType, zoneId);
        final String activityType = (String) params.get("activityType");
        final Integer minQuantity = (Integer) params.get("minQuantity");

        final List<Integer> domainIds = isSuperDomain(domainId)
                ? null
                : domainService.getChildDomainIds(domainId);

        final List<Object[]> report = cpeRepository.getDeviceEventReportExcel(domainIds, domainIds == null,
                manufacturer, model, from, to, activityType, minQuantity);
        ...
        final String path = ReportFileService.createExcelTable(userDomainId, report, DEVICE_EVENT, true,
